...
//添加子账号
func (receiver *AccountController) AddTeamSub() {
	...
	if _, err = dcm.GetDbSession().Table("dc_user_vip").Where("user_id=? and platform=1", subUser.Id).Get(&subUserVip); err != nil {
		receiver.FailReturn(global.NewError(5000))
		return
	}
	if subUserVip.Level != 0 {
		receiver.FailReturn(global.NewMsgError("专业版账号无法添加"))
	}
	var userVip dcm.DcUserVip
	if _, err = dcm.GetDbSession().Table("dc_user_vip").Where("user_id=? and platform=1", subUser.Id).Get(&userVip); err != nil {
		receiver.FailReturn(global.NewError(5000))
		return
	}
	var subCount int
	if _, err = dcm.GetDbSession().Table("dc_user").Where("parent_id=?", receiver.UserId).Count(&subCount); err != nil {
		receiver.FailReturn(global.NewError(5000))
		return
	}
	if subCount >= userVip.SubNum {
		receiver.FailReturn(global.NewMsgError("人数已满"))
		return
	}
	if _, err := dcm.UpdateInfo(dcm.GetDbSession(), subUser.Id, map[string]interface{}{"parent_id": receiver.UserId}, new(dcm.DcUserVip)); err != nil {
		return
	}
	receiver.SuccReturn(nil)
}
