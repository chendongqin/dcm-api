...
func (p *THBaseServiceClient) GetMultiple(ctx context.Context, table []byte, tgets []*TGet) (r []*TResult_, err error) {
	...
	defer func() {
		...
		if spendTime >= time.Second {
			logs.Warn("[hbase] [GetMultiple] [slow query] [table:%s] [size:%d] [took:%s]", string(table), len(tgets), spendTime)
		} else {
			logs.Debug("[hbase] [GetMultiple] [table:%s] [size:%d] [took:%s]", string(table), len(tgets), spendTime)
		}
	}()
